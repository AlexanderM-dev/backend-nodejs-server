...
async function start() {
    try {
        ...
        // TODO. Код инициализации должен выполняться 1 раз при пустой базе. !!!!! ГОТОВО, сделал через CREATE TABLE IF NOT EXISTS
        // TODO. Приложение должно понимать какой версии структура базы данных  !!!!! ГОТОВО
        //       (например, когда БД использовалась с новой версией приложения, затем, приложение откатили назад, а базу оставили как есть)
        // TODO. А что если случится ошибка при создании какой-то таблицы? При повторном запуске сервера что будет? !!!!!!!!ГОТОВО, сделал транзакцию
        // TODO. В коде почти нет комментариев, должно быть гораздо больше комментариев, на каждую функцию и практикуют документирование даже свойств в моделях. !!!! Не вижу смылса текущие функции описываьб.. Вроде и так всё понятно
